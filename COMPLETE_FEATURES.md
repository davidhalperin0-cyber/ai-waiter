# 📋 רשימה מלאה של כל מה שהאתר עושה כרגע

## 🎯 סקירה כללית

מערכת SaaS מלאה להזמנות דיגיטליות דרך QR codes עם AI assistant, ניהול מנויים, והדפסה אוטומטית.

---

## 🏗️ תשתית טכנית

### Frontend & Backend

- **Next.js 14** (App Router) + **React 18** + **TypeScript**
- **Tailwind CSS** לעיצוב
- **Framer Motion** לאנימציות
- **react-hot-toast** להתראות UX

### בסיס נתונים

- **Supabase (PostgreSQL)** - כל הנתונים נשמרים ב-Supabase
- **4 טבלאות עיקריות:**
  - `businesses` - פרטי עסקים, מנויים, הגדרות
  - `tables` - שולחנות וקודי QR
  - `menuItems` - פריטי תפריט
  - `orders` - הזמנות

### אבטחה

- **JWT** לאימות משתמשים
- **Middleware** מגן על `/dashboard` ו-`/super-admin`
- **Cookies** לשמירת סשן

---

## 👤 זרימת בעל העסק

### 1. רישום עסק חדש (`/register`)

- טופס רישום:
  - שם עסק
  - סוג עסק (בר/פיצריה/סושי/כללי)
  - תבנית עיצוב (Bar/Pizza/Sushi/Generic)
  - אימייל וסיסמה
- יוצר עסק חדש עם:
  - `businessId` ייחודי (UUID)
  - מנוי trial עם 10 שולחנות
  - `isEnabled = true`

### 2. התחברות (`/login`)

- טופס התחברות עם אימייל וסיסמה
- בדיקת סיסמה עם bcrypt
- יצירת JWT token ושמירה בקוקי
- הפניה אוטומטית לדשבורד

### 3. דשבורד עסק (`/dashboard`)

#### טאב 1: ניהול תפריט

- **הוספת/עריכת מנות:**
  - קטגוריה
  - שם מנה
  - מחיר
  - URL תמונה
  - מרכיבים (מופרדים בפסיקים)
  - אלרגנים (מופרדים בפסיקים)
  - ⭐ **מנה מומלצת** (checkbox) - מופיעה ב-carousel
  - 🤰 **מתאים להריון** (checkbox) - תג מיוחד בתפריט
- **רשימת מנות:**
  - תצוגה מסודרת עם תמונות
  - כפתור ⭐ מהיר להפיכה למומלצת
  - כפתור עריכה ומחיקה
  - תג "⭐ מומלץ" למנות מומלצות
- **API:**
  - `GET /api/menu` - טעינת מנות
  - `POST /api/menu` - יצירת מנה
  - `PUT /api/menu/[menuItemId]` - עדכון מנה
  - `DELETE /api/menu/[menuItemId]` - מחיקת מנה

#### טאב 2: שולחנות וקודי QR

- **הוספת שולחן:**
  - `tableId` - מזהה טכני (ללא רווחים)
  - `label` - שם שהלקוח רואה
- **QR Code:**
  - יצירה אוטומטית של QR code
  - קישור ישיר: `/menu/[businessId]/[tableId]`
  - אפשרות להורדה והדפסה
- **רשימת שולחנות:**
  - תצוגה של כל השולחנות
  - כפתור "צפה ב-QR" לכל שולחן
  - כפתור מחיקה
- **API:**
  - `GET /api/tables` - טעינת שולחנות
  - `POST /api/tables` - יצירת שולחן
  - `DELETE /api/tables/[tableId]` - מחיקת שולחן
- **לוגיקת מנוי:**
  - חסימת הוספת שולחנות אם `tables.length >= tablesAllowed`
  - הודעת שגיאה ברורה

#### טאב 3: הגדרות עסק

- **פרטי עסק:**
  - שם העסק
  - סוג עסק (`type`)
  - תבנית עיצוב (`template`) - משפיע על Theme
- **🤖 הוראות מותאמות ל-AI:**
  - שדה טקסט ארוך
  - מאפשר לעסק להגדיר הוראות ספציפיות ל-AI
  - לדוגמה: "בסושי, מנות X ו-Y חייבות להיות אפויות, לא נא"
- **סטטוס מנוי:**
  - תצוגת סטטוס (trial/active/expired/past_due)
  - מספר שולחנות בשימוש / מותר
  - תאריך חיוב הבא
  - כפתור "שדרג / חדש מנוי" → מעבר ל-Stripe Checkout
- **API:**
  - `GET /api/business/info` - טעינת פרטי עסק
  - `PUT /api/business/update` - עדכון פרטי עסק

#### טאב 4: הגדרות מדפסת / BON

- **הגדרות מדפסת:**
  - Enable/Disable מדפסת
  - סוג חיבור: HTTP/HTTPS, TCP, Serial
  - Endpoint (URL/IP)
  - Port (ל-TCP/Serial)
  - Payload Type: JSON, Text, XML
  - Headers מותאמים אישית (ל-HTTP)
- **בדיקת מדפסת:**
  - כפתור "בדוק מדפסת" - שולח הזמנת בדיקה
  - הודעת הצלחה/שגיאה
- **API:**
  - `PUT /api/business/printer-config` - שמירת הגדרות
  - `POST /api/printer/test` - בדיקת מדפסת

#### טאב 5: הזמנות ורווחים

- **טבלת הזמנות:**
  - כל ההזמנות של העסק
  - מידע: שולחן, סכום, סטטוס, תאריך
  - סיכום AI (אם קיים)
- **סיכומי רווחים:**
  - רווחים היום
  - רווחים השבוע
  - רווחים החודש
- **API:**
  - `GET /api/orders/list` - רשימת הזמנות
  - `GET /api/orders/stats` - סטטיסטיקות רווחים

---

## 🛒 זרימת לקוח

### 1. תפריט לקוח (`/menu/[businessId]/[tableId]`)

#### תצוגה ראשית (כש-`activeCategory === 'all'`)

- **Hero Section:**
  - Carousel "מבצעים ודילים מומלצים" - מנות עם `isFeatured = true`
  - Auto-rotate כל 4 שניות
  - תמונה גדולה, שם, מרכיבים, מחיר
  - כפתור "הוסף לעגלה"
  - קטגוריות בצד ימין (דסקטופ) או בחלק העליון (מובייל)
- **קטגוריות:**
  - **מובייל:** Sticky בחלק העליון, גלילה אופקית
  - **דסקטופ:** Sidebar מימין, sticky
  - כפתור "הכל" + כל הקטגוריות

#### תצוגת קטגוריה (כש-`activeCategory !== 'all'`)

- **Carousel קומפקטי** בראש (אם יש מנות מומלצות)
- **רשימת מנות:**
  - כרטיסי מנות מסודרים
  - תמונה, שם, קטגוריה, מחיר
  - מרכיבים ואלרגנים
  - תג "⭐ מומלץ" למנות מומלצות
  - תג "🤰 מתאים להריון" למנות מתאימות
  - כפתור "הוסף לעגלה"

#### הרחבת מנות (Expanded View)

- **לחיצה על מנה:**
  - **מובייל:** תצוגה כמעט מלאה, optimized ליד אחת
  - **דסקטופ:** הרחבה במקום, בתוך הפריסה
- **תוכן מורחב:**
  - תמונה גדולה יותר
  - תיאור מלא (מרכיבים, אלרגנים)
  - תג "מתאים להריון" אם רלוונטי
  - כפתור "הוסף לעגלה" בולט
- **סגירה:**
  - כפתור X
  - לחיצה מחוץ לכרטיס (מובייל)
  - החלקה למטה (מובייל)
  - שמירת מיקום גלילה

#### עגלה (Cart)

- **תחתית קבועה:**
  - סיכום כמות פריטים
  - סכום כולל
  - כפתור "סוגרים הזמנה עם העוזר החכם – שאלות, שינויים ומה שבא לכם →"
- **CartContext:**
  - ניהול גלובלי של העגלה
  - פונקציות: `addItem`, `removeItem`, `clearCart`

#### Theme System

- **4 תמות אנימטיביות:**
  - **Bar Theme:** אנימציות בירה, חלקיקים
  - **Pizza Theme:** אנימציות פיצה, גלים
  - **Sushi Theme:** אנימציות סושי, דגים
  - **Generic Theme:** אנימציות כלליות, חלקיקים
- **ThemeWrapper** בוחר את התמה לפי `business.template`

#### לוגיקת מנוי

- **בדיקת תוקף:**
  - `GET /api/menu/info` בודק סטטוס מנוי
  - אם `isEnabled = false` → 403 "Business is disabled"
  - אם `subscription.status = 'expired'` או `'past_due'` → 403 "Subscription expired"
- **הודעת אזהרה:**
  - באנר אדום בולט בתפריט
  - "המנוי פג תוקף" / "החשבון אינו פעיל"
  - חסימת הוספה לעגלה
  - הודעה: "התפריט מוצג לצפייה בלבד"

### 2. צ'אט AI (`/menu/[businessId]/[tableId]/chat`)

#### ממשק צ'אט

- **הודעות:**
  - תצוגת היסטוריית שיחה
  - הודעות משתמש (מימין)
  - הודעות AI (משמאל)
- **קלט:**
  - שדה טקסט להקלדת שאלות
  - כפתור שליחה
- **סטטוס:**
  - Loading state בעת שליחה
  - הודעות שגיאה אם יש בעיה

#### פונקציונליות AI

- **טעינת נתונים:**
  - `aiInstructions` של העסק
  - כל פריטי התפריט (כולל `isPregnancySafe`)
  - העגלה הנוכחית
- **System Prompt:**
  - עובדה: לא להמציא מנות
  - עזרה עם אלרגיות, מרכיבים, סוכר, גלוטן
  - עזרה בהריון - משתמש ב-`isPregnancySafe` והוראות העסק
  - סיכום הזמנה לפני אישור
  - הוראות מותאמות מהעסק
- **Actions JSON:**
  - AI יכול לשלוח `ACTIONS_JSON` עם:
    - `add_to_cart` - הוספת מנה לעגלה
    - `remove_from_cart` - הסרת מנה מהעגלה
  - הודעת אישור אוטומטית אחרי פעולה

#### אישור הזמנה

- **כפתור "Confirm Order":**
  - מופיע כשהזמנה מוכנה
  - שולח `POST /api/orders`
- **יצירת הזמנה:**
  - חישוב `totalAmount`
  - יצירת `orderId` (UUID)
  - שמירה ב-`orders` עם:
    - `businessId`, `tableId`
    - `items` (JSON)
    - `aiSummary` (אם קיים)
    - `status = 'received'`
- **אחרי אישור:**
  - ניקוי העגלה
  - הודעת הצלחה עם מזהה הזמנה
  - אפשרות להתחיל הזמנה חדשה

---

## 👨‍💼 סופר אדמין (`/super-admin`)

### 1. סקירה כללית

- **סטטיסטיקות:**
  - סך כל העסקים
  - עסקים פעילים
  - סך כל ההזמנות
  - הזמנות היום
  - סך כל ההכנסות
  - סך כל השולחנות
- **API:** `GET /api/super-admin/stats`

### 2. ניהול עסקים

- **טבלת עסקים:**
  - שם עסק, אימייל, סוג
  - מספר שולחנות (`tablesCount`)
  - מספר הזמנות (`ordersCount`)
  - סטטוס עסק (פעיל/מושבת)
  - סטטוס מנוי (trial/active/expired/past_due)
  - הוראות AI (אם קיימות)
- **פעולות:**
  - כפתור "השבת/הפעל" → מעדכן `isEnabled`
  - Dropdown סטטוס מנוי → מעדכן `subscription.status`
  - שדה מספר שולחנות → מעדכן `subscription.tablesAllowed`
- **API:**
  - `GET /api/super-admin/businesses` - רשימת עסקים
  - `PUT /api/super-admin/businesses/[businessId]` - עדכון עסק

### 3. תמחור

- **הגדרת מחיר:**
  - שדה `pricePerTable` (50₪ לחודש לכל שולחן)
  - חישוב דוגמאות (5/10/20 שולחנות)
- **עדכון שולחנות:**
  - שדה מספרי לכל עסק
  - On blur → עדכון `subscription.tablesAllowed`

---

## 💳 תשלומים (Stripe) - חלקי

### מה יש:

- **API Route:** `/api/billing/create-checkout-session`
- **יצירת Checkout Session:**
  - מקבל `businessId` ו-`tablesRequested`
  - מחשב מחיר: `tablesRequested * 50 ILS`
  - יוצר Stripe Checkout Session
  - מחזיר URL להפניה
- **כפתור בדשבורד:**
  - "שדרג / חדש מנוי"
  - מעבר ל-Stripe Checkout

### מה חסר:

- Webhooks לעדכון `subscription.status` אוטומטית
- עדכון `nextBillingDate` אוטומטית
- חישוב מחיר דינמי לפי `pricePerTable`

---

## 🎨 תכונות UX מתקדמות

### 1. התראות (Toasts)

- **react-hot-toast** להתראות
- **התראות במקומות:**
  - העתקת קישור QR
  - עדכון פרטי עסק
  - עדכון הגדרות מדפסת
  - הוספה/מחיקת מנה
  - הוספה/מחיקת שולחן
  - הוספה לעגלה
  - פעולות AI (הוספה/הסרה מהעגלה)

### 2. אנימציות

- **Framer Motion** בכל מקום:
  - מעברי דפים
  - כרטיסי מנות
  - Carousel
  - הרחבת מנות
  - כפתורים

### 3. Responsive Design

- **מובייל:**
  - קטגוריות sticky בחלק העליון
  - גלילה אופקית לקטגוריות
  - תצוגה מורחבת כמעט מלאה
  - תפריט מותאם למגע
- **דסקטופ:**
  - Grid layout
  - Sidebar קטגוריות
  - הרחבה במקום
  - תצוגה מיטבית

---

## 🔒 אבטחה ולוגיקה

### 1. אימות

- **JWT Tokens:**
  - יצירה ב-`/api/auth/login`
  - בדיקה ב-`middleware.ts`
  - שמירה בקוקי `auth`
- **Route Protection:**
  - `/dashboard/*` - דורש אימות
  - `/super-admin` - דורש אימות (TODO: בדיקת role)

### 2. בדיקות מנוי

- **ב-API:**
  - `GET /api/menu/info` - בודק `isEnabled` ו-`subscription.status`
  - מחזיר 403 אם לא תקין
- **בתפריט לקוח:**
  - חסימת הוספה לעגלה
  - הודעת אזהרה בולטת

### 3. הגבלות שולחנות

- **בדשבורד:**
  - בדיקה: `tables.length >= tablesAllowed`
  - חסימת הוספת שולחנות אם הגעה למגבלה
  - הודעת שגיאה ברורה

---

## 📊 API Routes מלאים

### Business

- `GET /api/business/info` - פרטי עסק
- `PUT /api/business/update` - עדכון עסק
- `POST /api/business/register` - רישום עסק
- `PUT /api/business/printer-config` - הגדרות מדפסת

### Menu

- `GET /api/menu` - רשימת מנות
- `POST /api/menu` - יצירת מנה
- `GET /api/menu/info` - פרטי עסק (לתפריט)
- `PUT /api/menu/[menuItemId]` - עדכון מנה
- `DELETE /api/menu/[menuItemId]` - מחיקת מנה

### Tables

- `GET /api/tables` - רשימת שולחנות
- `POST /api/tables` - יצירת שולחן
- `DELETE /api/tables/[tableId]` - מחיקת שולחן

### Orders

- `GET /api/orders/list` - רשימת הזמנות
- `GET /api/orders/stats` - סטטיסטיקות רווחים
- `POST /api/orders` - יצירת הזמנה

### AI

- `POST /api/ai/chat` - צ'אט עם AI

### Auth

- `POST /api/auth/login` - התחברות
- `GET /api/auth/me` - פרטי משתמש נוכחי

### Printer

- `POST /api/printer/test` - בדיקת מדפסת

### Billing

- `POST /api/billing/create-checkout-session` - יצירת Stripe Checkout

### Super Admin

- `GET /api/super-admin/stats` - סטטיסטיקות כלליות
- `GET /api/super-admin/businesses` - רשימת עסקים
- `PUT /api/super-admin/businesses/[businessId]` - עדכון עסק

---

## 🗄️ מבנה בסיס הנתונים

### Table: `businesses`

- `businessId` (TEXT, UNIQUE)
- `name`, `type`, `template`
- `email` (UNIQUE), `passwordHash`
- `isEnabled` (BOOLEAN)
- `subscription` (JSONB) - `{status, tablesAllowed, nextBillingDate}`
- `printerConfig` (JSONB) - הגדרות מדפסת
- `aiInstructions` (TEXT) - הוראות ל-AI
- `createdAt` (TIMESTAMPTZ)

### Table: `tables`

- `businessId` (TEXT, FK)
- `tableId` (TEXT)
- `label` (TEXT)
- UNIQUE(`businessId`, `tableId`)

### Table: `menuItems`

- `businessId` (TEXT, FK)
- `category` (TEXT)
- `name` (TEXT)
- `price` (NUMERIC)
- `imageUrl` (TEXT, nullable)
- `ingredients` (JSONB) - array
- `allergens` (JSONB) - array
- `is_featured` (BOOLEAN) - מנה מומלצת
- `is_pregnancy_safe` (BOOLEAN) - מתאים להריון
- UNIQUE(`businessId`, `name`)

### Table: `orders`

- `orderId` (TEXT, UNIQUE)
- `businessId` (TEXT, FK)
- `tableId` (TEXT)
- `items` (JSONB) - array of items
- `aiSummary` (TEXT, nullable)
- `status` (TEXT) - 'received', 'sent_to_printer', 'printed', 'printer_error'
- `totalAmount` (NUMERIC)
- `createdAt` (TIMESTAMPTZ)

---

## ✅ מה עובד בפועל

1. ✅ רישום עסק חדש + התחברות
2. ✅ אימות עם JWT וקוקי
3. ✅ דשבורד עסק מלא:
   - ניהול תפריט (CRUD + מנות מומלצות + מתאים להריון)
   - ניהול שולחנות וקודי QR
   - הגדרות עסק (כולל הוראות AI)
   - הגדרות מדפסת (UI + API)
   - צפייה בהזמנות ורווחים
4. ✅ תפריט לקוח מלא:
   - תפריט דינמי עם תמות אנימטיביות
   - Carousel מנות מומלצות
   - קטגוריות (sticky במובייל, sidebar בדסקטופ)
   - הרחבת מנות במקום
   - עגלה + סיכום
   - הודעת מנוי פג תוקף
5. ✅ צ'אט AI מלא:
   - עובד end-to-end עם OpenAI
   - רואה תפריט + הוראות מותאמות
   - עוזר עם אלרגיות, הריון, התאמות
   - יכול להוסיף/להסיר מהעגלה
   - יוצר הזמנה ושומר ב-Supabase
6. ✅ סופר אדמין:
   - סטטיסטיקות כלליות
   - ניהול עסקים
   - עדכון סטטוס מנוי ושולחנות
7. ✅ UX מתקדם:
   - Toasts להתראות
   - אנימציות חלקות
   - Responsive design מלא

---

## 🚧 מה עדיין חסר / חלקי

1. **Stripe Webhooks** - עדכון מנוי אוטומטי אחרי תשלום
2. **אינטגרציית מדפסת אמיתית** - שליחה אוטומטית למדפסת אחרי הזמנה
3. **התראות** - אימייל/WhatsApp על הזמנות חדשות
4. **דוחות מתקדמים** - Export ל-CSV, גרפים
5. **דף Subscription Expired** - עמוד ייעודי יפה

---

**סיכום:** המערכת היא פלטפורמה מלאה ופונקציונלית להזמנות דיגיטליות עם AI, ניהול מנויים, והדפסה. רוב התכונות עובדות, ונותרו בעיקר שיפורי UX ואינטגרציות חיצוניות.






