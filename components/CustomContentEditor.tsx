...
interface CustomContentEditorProps {
  ...
  onSave: (savedContent?: CustomContent) => Promise<void>;
}
...
export default function CustomContentEditor({ businessId, initialContent, onSave }: CustomContentEditorProps) {
  ...
  const handleSave = async () => {
    try {
      ...
      const savedContent = data.business?.customContent;
      console.log('✅ Save successful, savedContent from RPC:', savedContent);
      
      await onSave(savedContent);
      console.log('✅ Business info updated with savedContent');
